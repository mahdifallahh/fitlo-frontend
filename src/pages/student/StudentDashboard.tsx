...
export default function StudentDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 xl:grid xl:grid-cols-[auto,1fr]">
      {/* Sidebar */}
      <aside className={`fixed xl:static z-30 top-0 right-0 h-full xl:h-auto w-64 xl:w-56 bg-white shadow-lg flex flex-col py-8 px-4 transition-transform duration-200 ${sidebarOpen ? 'translate-x-0' : 'translate-x-full xl:translate-x-0'}`}>
        {/* Mobile close button */}
      ...
      {/* Overlay for mobile */}
      {sidebarOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-30 z-20 xl:hidden" onClick={() => setSidebarOpen(false)}></div>
      )}

      {/* Main Content */}
      <div className="flex-1 flex flex-col">
        {/* Header for mobile */}
        <header className="bg-white shadow xl:hidden sticky top-0 z-10">
        ...
        <main className="max-w-2xl mx-auto p-4 w-full justify-self-center">
